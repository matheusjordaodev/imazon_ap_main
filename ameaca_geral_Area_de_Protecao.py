...
roi = load_geojson("https://raw.githubusercontent.com/imazon-cgi/ap/main/dataset/geojson/AMEACA_GERAL_Area_de_Protecao.geojson")
roi['NOME'] = roi['NOME'].str.upper().apply(lambda x: unidecode.unidecode(x) if isinstance(x, str) else x)
roi = roi.sort_values(by='RANK')

df = load_df('https://github.com/imazon-cgi/ap/raw/refs/heads/main/dataset/csv/AMEACA_GERAL_Area_de_Protecao.parquet')
...
def update_graphs(modalidade, uso, states, reset_clicks, bar_click_data, map_click_data, selected_states):
    ...
    pie_uso_fig = px.pie(top_10, values='DESMATAM_1', names='UF', color='MODALIDADE', title='Ameaça Desmatamento por Estado de Uso e Categoria')
    pie_uso_fig.update_traces(textinfo='percent+label', marker=dict(colors=pie_colors))

    # Gráfico de pizza - Terra Indígena
    pie_unid_fig = px.pie(top_10, values='DESMATAM_1', names='NOME', color='UF', title='Ameaça Desmatamento por Terra Indígena')
