# app/dashboards/pressao_ucs.py
"""
Dashboard Dash – Pressão Geral em Unidades de Conservação (UCs)

Para registrar no Flask:

    from app.dashboards.pressao_ucs import register_pressao_ucs_dashboard

    def create_app():
        server = Flask(__name__)
        register_pressao_ucs_dashboard(server)   # rota: /pressao_ucs/
        return server
"""
...
# =============================================================================
def register_pressao_ucs_dashboard(flask_server):
    dash_app = dash.Dash(
        __name__,
        server=flask_server,
        url_base_pathname="/pressao_ucs/",
    ...
    dash_app.layout = dbc.Container(
        [
            ...
            dbc.Modal(
                [
                    dbc.ModalHeader(
                        dbc.ModalTitle("Escolha as UCs da Amazônia Legal")
                    ),
                    dbc.ModalBody(
                        [
                            dbc.Checklist(
    ...
    def update_graphs(
        ...
        # ---------- filtros ----------
        filtered_df = df[
            (df["MODALIDADE"] == modalidade) & (df["USO"] == uso)
        ]
        ...
        bar_fig.update_layout(
            ...
            font=dict(size=10),
            title=dict(
                text="Top 10 UCs por Pressão de Desmatamento",
                x=0.5,
                xanchor="center",
            ),
        )
