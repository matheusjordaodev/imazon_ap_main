# app/dashboards/ucs.py
"""
Dashboard Dash – Ameaça Geral em Unidades de Conservação (UCs)

Uso no Flask:
    from app.dashboards.ucs import register_ucs_dashboard

    def create_app():
        server = Flask(__name__)
        register_ucs_dashboard(server)   # rota: /ucs/
        return server
"""
...
df = load_df(
    "https://github.com/imazon-cgi/ap/raw/refs/heads/main/dataset/csv/AMEACA_GERAL_UCs.parquet"
)
...
# =============================================================================
def register_ucs_dashboard(flask_server):
    dash_app = dash.Dash(
        __name__,
        server=flask_server,
        url_base_pathname="/ucs/",
    ...
    def update_graphs(
        ...
        pie_uso_fig = px.pie(
            ...
            names="UF",
            color="CATEGORIA",
            title="Ameaça Desmatamento por Estado (categoria)",
        )
