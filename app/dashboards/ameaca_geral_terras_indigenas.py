# app/dashboards/ameaca_terras_indigenas.py
"""
Dashboard Dash – Ameaça Geral em Terras Indígenas
rota: /ameaca_terras_indigenas/
"""
...
def load_geojson(url: str):
    try:
        ...
        print(f"GeoPandas falhou ({e}) – tentando download manual…")
        try:
            ...
            print(f"Download manual do GeoJSON falhou: {e2}")
            return None
...
def load_df(url: str):
    try:
        ...
        print(f"Pandas falhou ({e}) – tentando download manual…")
        try:
            r = requests.get(url, headers=HEADERS, timeout=30)
            r.raise_for_status()
            return pd.read_parquet(io.BytesIO(r.content))
        except Exception as e2:
            print(f"Download manual do Parquet falhou: {e2}")
            return None

# ----------------------------------------------------------------------
# Carrega datasets Ameaça Geral – Terras Indígenas
# ----------------------------------------------------------------------
roi = load_geojson(
    "https://raw.githubusercontent.com/imazon-cgi/ap/main/dataset/geojson/"
    "AMEACA_GERAL_Terra_indigena.geojson"
)
if roi is None:
    raise RuntimeError("Falha ao carregar GeoJSON de Ameaça TI.")
...
df = load_df(
    "https://github.com/imazon-cgi/ap/raw/refs/heads/main/dataset/csv/"
    "AMEACA_GERAL_Terra_indigena.parquet"
)
if df is None:
    raise RuntimeError("Falha ao carregar Parquet de Ameaça TI.")
...
def register_ameaca_terra_indigena_dashboard(flask_server):
    ...
    def atualizar(modalidade, fase, uf, reset, bar_click, map_click, selecionados):
        ...
        for click_data in (bar_click, map_click):
            if click_data:
                nome = (click_data["points"][0].get("y") or
                        click_data["points"][0].get("location"))
                if nome in (selecionados or []):
                    selecionados.remove(nome)
                else:
                    selecionados = (selecionados or []) + [nome]
        ...
        tbody = html.Tbody([
            html.Tr([
                html.Td(r["NOME"]), html.Td(r["FOCOS DE C"]),
                html.Td(r["N DE CAR"]), html.Td(f"{r['CAR']:.2f} km²"),
        ...
        bar.update_layout(
            xaxis_title="Área (km²)", yaxis_title="Terras Indígenas",
