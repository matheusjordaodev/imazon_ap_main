...
dbc.Row([
    dbc.Col(dbc.Card([
        dbc.CardBody([
            html.H1("Análise de Pressão de Desmatamento - Amazônia Legal", className="text-center mb-4"),
            dbc.Row([
                dbc.Col(html.Label('Modalidade:', className="fw-bold"), width='auto', className="align-self-center"),
                dbc.Col(dcc.Dropdown(id='modalidade-dropdown', options=modalidade_options,multi=True, placeholder="Selecione a modalidade" ), width=3),
                dbc.Col(html.Label('Uso:', className="fw-bold"), width='auto', className="align-self-center"),
                dbc.Col(dcc.Dropdown(id='uso-dropdown', options=uso_options, multi=True, placeholder="Selecione o uso" ), width=3),
...
dcc.Download(id="download-dataframe-csv"),
    ...
    dcc.Download(id="download-dataframe-csv"),
  dbc.Modal([
        dbc.ModalHeader(dbc.ModalTitle("Escolha Área de Proteção Ambiental da Amazônia Legal")),
        dbc.ModalBody([
            dcc.Dropdown(
    ...
    dbc.Modal([
        dbc.ModalHeader(dbc.ModalTitle("Escolha as Área de Proteção Ambiental da Amazônia Legal")),
        dbc.ModalBody([
            dbc.Checklist(
                options=state_options,
                id="state-checklist",
...
def update_graphs(modalidade, uso, states, reset_clicks, bar_click_data, map_click_data, selected_states):
    ...
    bar_fig.update_layout(
        ...
        font=dict(size=10),
        title={
            'text': f'Top 10 Área de Proteção Ambiental por Desmatamento',
            'x': 0.5,
            'xanchor': 'center',
            'yanchor': 'top'
        }
    ...
    map_fig.update_layout(
        title={
            'text': f"Mapa de Pressão de Desmatamento (km²)",
            'x': 0.5,
            'xanchor': 'center',
            'yanchor': 'top',
            'font': {'size': 14}
    ...
    pie_uso_fig = px.pie(top_10, values='DESMATAM_1', names='UF', color='MODALIDADE', title='Pressão Desmatamento por Estado de Uso e Categoria')
    pie_uso_fig.update_traces(textinfo='percent+label', marker=dict(colors=pie_colors))

    # Gráfico de pizza - Terra Indígena
    pie_unid_fig = px.pie(top_10, values='DESMATAM_1', names='NOME', color='UF', title='Pressão Desmatamento por Terra Indígena')

    # Remover os valores dos ticks e manter apenas a legenda e hover
    pie_unid_fig.update_traces(
        textinfo='none',  # Não exibe os valores nos setores
        hoverinfo='label+value+percent',  # Mostra nome, valor e percentual no hover
