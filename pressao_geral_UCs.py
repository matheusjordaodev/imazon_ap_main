...
def update_graphs(modalidade, uso, states, reset_clicks, bar_click_data, map_click_data, selected_states):
    ...
    pie_uso_fig = px.pie(top_10, values='DESMATAM_1', names='UF', color='CATEGORIA', title='Pressão Desmatamento por Estado de Uso e Categoria')
    pie_uso_fig.update_traces(textinfo='percent+label', marker=dict(colors=pie_colors))

    # Gráfico de pizza - Unidade de Conservação
    pie_unid_fig = px.pie(top_10, values='DESMATAM_1', names='NOME', color='USO', title='Pressão Desmatamento por Unidade de Conservação')
